...
**Resultado de la Fase 6 (SLAM, 2026-09-24):**
- `pepper_gazebo_plugin/config/pepper_slam_toolbox.yaml` (online async, parámetros de
  gmapping del V8 trasladados) y `launch/pepper_slam_toolbox.launch.py` (args `scan`,
  `max_laser_range`). **slam_toolbox en Jazzy es lifecycle**: el launch emite configure y
  activate; si no, se queda en `unconfigured` y no publica `/map`.
- **Bug del V8 en laser_publisher.py**: `angle_max = angle_min + 488·inc` con 488 rayos;
  karto exige `+487·inc` ("contains 488 range readings, expected 489"). Corregido.
- Mapas en `pepper_gazebo_plugin/map/`: `office` (hokuyo, limpio) y `office_laser2`
  (punteado, con algo de deriva: sólo 45 puntos útiles por scan).
- `config/pepper_sensors_map.rviz` = sensores + mapa (fixed frame `map`), por
  `pepper_sensors_rviz.launch.py config:=pepper_sensors_map.rviz`.
- Recorrido automático de prueba (19 puntos, guiado con el ground truth) en el scratchpad de la
